...
void translate_main(CodeGen *codegen) {
    ...
    codegen->append_inst("bcnez $fcc0,label1");

    codegen->append_inst("addi.w $t0,$zero,0");
    codegen->append_inst("b label2");

    codegen->append_inst("label1", ASMInstruction::Label);
    codegen->append_inst("addi.w $t0,$zero,1");

    codegen->append_inst("label2", ASMInstruction::Label);
    ...
    codegen->append_inst("ld.w",
                         {"$t0", "$fp", std::to_string(offset_map["%op1"])});
    codegen->append_inst("xori $t1,$t0,1");
    codegen->append_inst("slti $t0,$t1,1");
    codegen->append_inst("st.b",
